 ...
 */
const logGroups = () => W.logGroups || [];
const currentLogGroup = () => W.currentLogGroup || {};
/**
 ...
 */
export function logGroupOpen(name, ...msgs) {
 ...
 */
export function logGroup(name, ...msgs) {
...
export function printMemory() {
    ...
    const savedMemoryUsage = W.memoryUsage;
    if (memory) {
        let memoryUsageMSG = {
            MaxUsage: formatBytes(memory.totalJSHeapSize),
            Using: formatBytes(memory.usedJSHeapSize),
            MaxItemSize: formatBytes(memory.jsHeapSizeLimit)
        };
        W.memoryUsage = memory;
        if (savedMemoryUsage) {
            const difference = memory.usedJSHeapSize - savedMemoryUsage.usedJSHeapSize;
            memoryUsageMSG = {
                improved: difference < 0,
                difference: formatBytes(difference),
                was: {
                    MaxUsage: formatBytes(savedMemoryUsage.totalJSHeapSize),
                    Using: formatBytes(savedMemoryUsage.usedJSHeapSize),
...
export function memoryPerformance() {
    const w_performance = performance;
    if (!w_performance || w_performance.memory === undefined)
        return undefined;
 ...
 * not return a value.
 */
export function logGroupInner(open, name, ...msgs) {
    ...
    if ((current === null || current === void 0 ? void 0 : current.depth) > 50) {
    ...
    const group = {
        ...
        name,
        depth: !current || !current.depth ? 1 : current.depth + 1,
        open,
        msgs,
        parent: current,
        children: [],
    ...
    if (current) {
        current === null || current === void 0 ? void 0 : current.children.push(group);
    }
    else {
...
export function clearLogGroups() {
    ...
    CONSOLELOG('Clearing log groups', {
        logGroups: [...logGroups()],
        currentLogGroup: Object.assign({}, currentLogGroup())
    });
    if (logGroups()) {
 ...
 */
export function logGroupEnd(...remarks) {
 ...
 */
export function closeAllLogGroups(currentGroups = getAllOpenGroups()) {
 ...
 * @returns an array of LogGroup objects.
 */
export function openAllLogGroups(currentGroups = getAllOpenGroups()) {
 ...
 */
export function globalLog(...message) {
 ...
 * @returns an array of LogGroup objects.
 */
export function getAllOpenGroups() {
    if (!W.logGroups || !W.currentLogGroup)
        return [];
    const currentGroups = [];
    const currentGroup = W.currentLogGroup;
    if (!currentGroup)
