    ...
    if (node.hasChildNodes()) {
        for (let i = 0; i < node.childNodes.length; i++) {
            ...
            if (childNode.nodeType === 1) {
                ...
                // Text node
                const textContent = (_a = childNode.nodeValue) === null || _a === void 0 ? void 0 : _a.trim();
                if (textContent) {
...
export function areObjectsEqual(x, y) {
    ...
    return (Object.keys(y).every((i) => {
        ...
        p.every((i) => areObjectsEqual(x[i], y[i])));
}
/**
 ...
 */
export function safelyParse(text) {
    try {
        if (nullIsh(text))
            return undefined;
 ...
 */
export function safelyStringify(object, space = undefined) {
    if (object === undefined)
        return undefined;
 ...
 */
export function removeProperties(obj, propName) {
    if (Array.isArray(obj)) {
        ...
        for (const key in obj) {
            if (key === propName) {
                ...
                removeProperties(obj[key], propName);
            }
        }
    }
}
...
export function Obj(object) {
    return {
        has: (key) => hasProperty(object, key),
        get: (key) => hasProperty(object, key) ? object[key] : undefined,
        set: (key, value) => {
            object[key] = value;
            return object;
        }
 ...
 * `spec`.
 */
export function updateObject(object, spec) {
            ...
            switch (commandKey) {
                ...
                case '$merge':
                    if (typeof newObj === 'object') {
                        newObj = Object.assign(Object.assign({}, newObj), spec[commandKey]);
                    }
                    break;
                case '$apply':
 ...
 * @returns an object of type UnknownObject.
 */
export function moveProp(obj, key, direction) {
 ...
 */
export function reorderObject(obj, orderedKeys) {
    const reorderedObj = {};
 ...
 */
export function convertListToMap(list, keyName, handleDuplicates = false) {
    const map = {};
 ...
 */
export function selectFieldsMap(map, fields, hideNull = true) {
    const newMap = {};
 ...
 */
export function selectFields(object, fields, hideNull = true) {
