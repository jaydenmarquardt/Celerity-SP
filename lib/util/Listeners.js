...
const LISTENER_KEY = "hnsListeners";
/**
 ...
 */
function triggerEvent(key, ...props) {
 ...
 * `oldParams`.
 */
export function onUrlParamsChange(callback) {
  ...
  if (!urlParamListener) {
    urlParamListener = setInterval(() => {
      ...
      const oldOldParams = Object.assign({}, oldParams);
...
export function onEditModeChange(callback) {
  onUrlParamsChange((params, oldParams) => {
    const wasEdit =
      (oldParams === null || oldParams === void 0 ? void 0 : oldParams.Mode) ===
      "Edit";
    const isEdit =
      (params === null || params === void 0 ? void 0 : params.Mode) === "Edit";
    if (wasEdit && !isEdit) {
      callback(false);
    }
    if (!wasEdit && isEdit) {
