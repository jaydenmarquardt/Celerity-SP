...
import { getDeviceType, getOperatingSystem, getBrowser } from "./Device";
import { safelyStringify } from "./Objects";
 ...
 */
export function setDebugEnabled(enable: boolean) {
  W["splibDebug"] = enable;
}
 ...
 */
function colorLog(hexColor: string, ...args): void {
 ...
 */
export function debug(...log): void {
  if (isDebugEnabled()) {
...
export interface IErrorLog {
  ...
  message: string;
  component?: string; //The File and or function
  data?: any;
}

export function errorLog(logg: IErrorLog) {
  const log: IErrorLog = {
    type: "error",
 ...
 */
export function success(...log: any) {
  if (isDebugEnabled()) {
