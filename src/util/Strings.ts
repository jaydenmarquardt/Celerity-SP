 ...
 */
export function nicefyString(input: string): string {
  return input
    .split('-')
 ...
 * empty or not.
 */
export function emptyString(text?: string | boolean): boolean {
  if (text === undefined || text === null || text === false) return true;
  if (text === 'undefined' || text === 'null') return true;
  if (text.toString().trim().length === 0) return true;
  return false;
}
 ...
 * being returned.
 */
export function captitalise(text: string): string {
  if (!text) return text;
 ...
 */
export function capitaliseEachWord(sentence: string): string {
  if (!sentence) return '';
 ...
 */
export function replaceBackToIllegalCharacters(str: string): string {
  return (
    str
      // replaceAll(";", ",").
      .replaceAll('\uFF02', '"')
...
export function price(value: number | string): string {
  if (!value) return '$-';
  let formattedPrice: string = price?.toString().replaceAll('$', '');
  try {
    formattedPrice = `$${
      this.item.ClassifiedPrice?.toFixed(2).replace(
...
export function stripHtml(maybeHasHtmlString: string): string {
