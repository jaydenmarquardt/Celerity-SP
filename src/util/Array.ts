...
export function filterArrayByProperty(
  ...
  return arr.filter((el: UnknownObject) => {
 ...
 */
export function hasProperty(object: UnknownObject, property: string): boolean {
...
export function findByProperty(
  ...
  const result = arr.filter((el: UnknownObject) => {
    if (value === '<EXISTS>' && !!el[prop]) return true;
    return el[prop] === value;
  });
...
export function paginate(
  ...
  return items.reduce((acc: UnknownObject, val: UnknownObject, i: number) => {
...
export function paginatePage(
  ...
  return paginate(array, perPage)?.[page_number] || [];
}
 ...
 */
export function moveArrayItemPosition(
  arr: unknown[],
  old_index: number,
  new_index: number
): unknown[] {
 ...
 */
export function unSerialiseArray(string: string): string[] {
  string = string
    .replaceAll('+', ' ')
    .replaceAll('+', ' ')
    .replaceAll('%2C', ',');
 ...
 */
export function transformToAssocArray(prmstr: string): UnknownObject {
 ...
 */
export function arrayMoveIndex(
  arr: unknown[],
  fromIndex: number,
  toIndex: number
): void {
 ...
 */
export function groupBy(items: UnknownObject[], key: string): UnknownObject {
  return items.reduce(
    (result: UnknownObject, item: UnknownObject) => ({
      ...result,
