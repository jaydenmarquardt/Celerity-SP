...
var DEFAULT_REFRESH_THRESHOLD_MS = 10 * 60 * 1000; // 10 mins
var DEFAULT_MAX_STORE_SIZE = 15;
/**
...
var NavigationDataStore = /** @class */ (function () {
    function NavigationDataStore(id, buildIdKey, buildId, executor, validator, refreshThresholdMs, maxStoreSize) {
        ...
        this._refreshThreshold = refreshThresholdMs || DEFAULT_REFRESH_THRESHOLD_MS;
        this._maxStoreSize = maxStoreSize || DEFAULT_MAX_STORE_SIZE;
    ...
    NavigationDataStore._upsertItemData = function (item, 
    ...
    NavigationDataStore.prototype._addOrUpdateCacheEntry = function (url, data, isNavData, cachedEntry) {
        if (cachedEntry && cachedEntry.item) {
            ...
            if (entry.item) {
                ...
                try {
                    // eslint-disable-next-line @rushstack/security/no-unsafe-regexp, no-new
                    new RegExp(url);
                }
                catch (error) {
    ...
    cachedItemIdx) {
        ...
        var prefetchedDataPromise;
        if (props && props.enableDataPrefetch) {
            var qosMonitor_1 = new sp_diagnostics_1._QosMonitor(fetchAppDataQosScenarioName);
            prefetchedDataPromise = this._executor(url, NavigationDataStore._generatePrefetchHeaders(props))
