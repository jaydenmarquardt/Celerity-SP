...
var Navigator = /** @class */ (function () {
        ...
        return this._navigationDataProvider
            ...
            return response.preloadedData
                ...
                if (response.prefetchedData) {
                    if (!(0, Killswitches_1.isMoveResetBeforeUpdatePrefetchKSActive)() && preloadedData.item) {
                        _this._raiseClearDataEvent(preloadedData.item.UniqueId);
                    }
                    else {
                        _this._raisePrefetchDataEvent(url, preloadedData, response.prefetchedData);
                    }
                }
                return _this.navigateToPreloadedData(preloadedData, url, response.prefetchedData);
            })
     ...
     * @param url - url that is used to sent prefetch data event
     */
    Navigator.prototype.navigateToPreloadedData = function (preloadedData, url, 
    ...
    prefetchData) {
        ...
        try {
            ...
            return this._navigationOrchestrator
                ...
                if (!(0, Killswitches_1.isMoveResetBeforeUpdatePrefetchKSActive)() && url && prefetchData) {
                    // make sure prefetch event is sent after reset event is sent
                    _this._raisePrefetchDataEvent(url, preloadedData, prefetchData);
                }
                qosMonitor.writeSuccess();
                return navigationResult;
            })
    ...
    Navigator.prototype._loadApplicationCustomizers = function (preloadedData) {
        return this._navigationOrchestrator._loadApplicationCustomizersAsync(preloadedData);
    };
    /**
    ...
    Navigator.prototype._raiseClearDataEvent = function (pageUniqueId) {
        sp_core_library_1._SPEventManager.instance.raiseStickyEvent('resetCacheManager', pageUniqueId);
        // Resetting legacy cache manager is required until all webparts migrate to Tiered Caching
        sp_core_library_1._SPEventManager.instance.raiseStickyEvent('resetLegacyCacheManager', pageUniqueId);
    };
    Navigator.prototype._raisePrefetchDataEvent = function (url, preloadedData, prefetchData) {
        void prefetchData.then(function (data) {
            if (data) {
