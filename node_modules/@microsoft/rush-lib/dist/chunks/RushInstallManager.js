...
class TempProjectHelper {
    ...
    createTempProjectTarball(rushProject) {
        _rushstack_node_core_library__WEBPACK_IMPORTED_MODULE_0__.FileSystem.ensureFolder(path__WEBPACK_IMPORTED_MODULE_2__.resolve(this._subspace.getSubspaceTempFolder(), 'projects'));
    ...
    getTarballFilePath(project) {
        return path__WEBPACK_IMPORTED_MODULE_2__.join(this._subspace.getSubspaceTempFolder(), _RushConstants__WEBPACK_IMPORTED_MODULE_3__.RushConstants.rushTempProjectsFolderName, `${project.unscopedTempProjectName}.tgz`);
    }
    getTempProjectFolder(rushProject) {
        const unscopedTempProjectName = rushProject.unscopedTempProjectName;
        return path__WEBPACK_IMPORTED_MODULE_2__.join(this._subspace.getSubspaceTempFolder(), _RushConstants__WEBPACK_IMPORTED_MODULE_3__.RushConstants.rushTempProjectsFolderName, unscopedTempProjectName);
    }
}
...
class RushInstallManager extends _base_BaseInstallManager__WEBPACK_IMPORTED_MODULE_6__.BaseInstallManager {
    ...
    async prepareCommonTempAsync(subspace, shrinkwrapFile) {
        ...
        const tempProjectsFolder = path__WEBPACK_IMPORTED_MODULE_1__.join(this.rushConfiguration.commonTempFolder, _logic_RushConstants__WEBPACK_IMPORTED_MODULE_9__.RushConstants.rushTempProjectsFolderName);
            ...
            allExplicitPreferredVersions.forEach((version, dependency) => {
                ...
                if (!shrinkwrapFile.hasCompatibleTopLevelDependency(dependencySpecifier)) {
                    shrinkwrapWarnings.push(`Missing dependency "${dependency}" (${version}) required by the preferred versions from ` +
                        _logic_RushConstants__WEBPACK_IMPORTED_MODULE_9__.RushConstants.commonVersionsFilename);
                    shrinkwrapIsUpToDate = false;
                }
            });
            ...
            if (orphanedProjects.length > 0) {
                for (const orphanedProject of orphanedProjects) {
                    shrinkwrapWarnings.push(`Your ${this.rushConfiguration.shrinkwrapFilePhrase} references "${orphanedProject}" ` +
                        `which was not found in ${_logic_RushConstants__WEBPACK_IMPORTED_MODULE_9__.RushConstants.rushJsonFilename}`);
                }
                shrinkwrapIsUpToDate = false;
            }
        }
        ...
        for (const rushProject of sortedRushProjects) {
            ...
            commonDependencies.set(rushProject.tempProjectName, `file:./${_logic_RushConstants__WEBPACK_IMPORTED_MODULE_9__.RushConstants.rushTempProjectsFolderName}/${rushProject.unscopedTempProjectName}.tgz`);
    ...
    canSkipInstall(lastModifiedDate, subspace) {
        ...
        return _utilities_Utilities__WEBPACK_IMPORTED_MODULE_10__.Utilities.isFileTimestampCurrent(lastModifiedDate, potentiallyChangedFiles);
    }
    /**
    ...
    async installAsync(cleanInstall, subspace) {
        ...
        const commonNodeModulesFolder = path__WEBPACK_IMPORTED_MODULE_1__.join(this.rushConfiguration.commonTempFolder, _logic_RushConstants__WEBPACK_IMPORTED_MODULE_9__.RushConstants.nodeModulesFolderName);
        ...
        if (_rushstack_node_core_library__WEBPACK_IMPORTED_MODULE_4__.FileSystem.exists(commonNodeModulesFolder)) {
            ...
            if (cleanInstall) {
                ...
                if (this.rushConfiguration.packageManager === 'npm') {
                    ...
                    _utilities_Utilities__WEBPACK_IMPORTED_MODULE_10__.Utilities.executeCommandWithRetry({
                    ...
                    const pathToDeleteWithoutStar = path__WEBPACK_IMPORTED_MODULE_1__.join(commonNodeModulesFolder, _logic_RushConstants__WEBPACK_IMPORTED_MODULE_9__.RushConstants.rushTempNpmScope);
                    // eslint-disable-next-line no-console
        ...
        if (this.rushConfiguration.packageManager === 'npm') {
            ...
            _utilities_Utilities__WEBPACK_IMPORTED_MODULE_10__.Utilities.executeCommand({
    ...
    async _fixupNpm5RegressionAsync() {
        const pathToDeleteWithoutStar = path__WEBPACK_IMPORTED_MODULE_1__.join(this.rushConfiguration.commonTempFolder, 'node_modules', _logic_RushConstants__WEBPACK_IMPORTED_MODULE_9__.RushConstants.rushTempNpmScope);
        // Glob can't handle Windows paths
        const normalizedPathToDeleteWithoutStar = _rushstack_node_core_library__WEBPACK_IMPORTED_MODULE_4__.Text.replaceAll(pathToDeleteWithoutStar, '\\', '/');
